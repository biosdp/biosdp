...
c = 1;
%%%%%%%%%%%%% HARD CODED  %%%%%%%%%%%
% sdpvar t1 x1 x2 x3 x4 x5 x6 x7 x8 u1;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% sosfeasability = {};
...
%        s = display(sos_solution(c+j-1));
%        eval(['pyalmip = ' s]);
%        try
%            [yalmip_sol,m_y,B_y,residual_Yalmip] = solvesos(sos(pyalmip),[])
%            sostest.feasability = yalmip_sol;
%            sostest.residual = residual_Yalmip;
%        catch
%            disp('NOT SOS !!');
%            sostest.feasability = -1;
%            sostest.residual = 'NaN';
%        end  
%        sosfeasability = [sosfeasability;sostest];
     end
%     s = display(sos_solution(c+infos_tmp.nbsos));
%     eval(['pyalmip = ' s]);
%     try
%        [yalmip_sol,m_y,B_y,residual_Yalmip] = solvesos(sos(pyalmip),[])
%        sostest.feasability = yalmip_sol;
